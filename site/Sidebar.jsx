...
function Sidebar() {
  ...
  return (
    <aside style={{
      ...
      width: 160,
      zIndex: 40,
      display: 'flex', flexDirection: 'column', justifyContent: 'center', paddingLeft: 4,
      opacity: visible ? 1 : 0,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
        ...
        />
        <div>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 14, color: 'var(--text-primary)', lineHeight: 1.1 }}>John Besco</div>
        </div>
      </div>

      <nav aria-label="Section navigation" style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
        {NAV.map(({ id, label }) => {
          const on = active === id;
          return (
            <button key={id} onClick={() => go(id)} aria-current={on ? 'page' : undefined}
              ...
              <span style={{
                display: 'block', borderRadius: 999, flex: 'none',
                width: on ? 7 : 5, height: on ? 7 : 5,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 28 }}>
        <span className="hero-online-dot" style={{ width: 6, height: 6, borderRadius: 999, background: 'var(--color-success)', boxShadow: '0 0 0 3px color-mix(in srgb, var(--color-success) 13%, transparent)' }} />
        <span style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text-secondary)' }}>Open to work</span>
      </div>
    </aside>
  );
}
