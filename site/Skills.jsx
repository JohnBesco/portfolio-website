...
const CUSTOM_SVG = {
  ...
  gohighlevel: (size) => (
    <svg width={size} height={size} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
      ...
      <path fill="#F5B731" d="M8 82 L8 50 L2 50 L20 18 L38 50 L32 50 L32 82 Z"/>
      {/* blue arrow — centre, shortest */}
      <path fill="#4B8EEA" d="M34 82 L34 58 L28 58 L46 32 L64 58 L58 58 L58 82 Z"/>
      {/* green arrow — right, same height as yellow */}
      <path fill="#3DC45A" d="M60 82 L60 50 L54 50 L72 18 L90 50 L84 50 L84 82 Z"/>
    </svg>
  ),
};
...
const STACK = [
  {
    title: 'AI / LLM',
    items: [
      { label: 'OpenAI', svgKey: 'openai' },
      { label: 'Anthropic (Claude Code, Skills & Workflows)', slug: 'anthropic', hex: 'D97757' },
      { label: 'ElevenLabs', slug: 'elevenlabs', hex: 'EDEDED' },
      { label: 'DeepSeek', slug: 'deepseek', hex: '4D9CF7' },
      { label: 'OpenCode', svgKey: 'opencode' },
    ],
  },
  {
    title: 'AUTOMATION',
    items: [
      { label: 'Zapier', slug: 'zapier', hex: 'FF4A00' },
      { label: 'Make', slug: 'make', hex: 'A259FF' },
    ],
  },
  {
    title: 'DEVELOPMENT',
    items: [
      { label: 'React', slug: 'react', hex: '61DAFB' },
      { label: 'Tailwind CSS', slug: 'tailwindcss', hex: '38BDF8' },
      { label: 'JavaScript', slug: 'javascript', hex: 'F7DF1E' },
      { label: 'HTML / CSS', slug: 'html5', hex: 'E34F26' },
      { label: 'WordPress', slug: 'wordpress', hex: '6CA8D9' },
      { label: 'Sass', svgKey: 'sass' },
      { label: 'Python', svgKey: 'python' },
    ],
  },
  {
    title: 'TOOLS & PLATFORMS',
    items: [
      { label: 'Figma', slug: 'figma', hex: 'F24E1E' },
      { label: 'Webflow', slug: 'webflow', hex: '4D8DF7' },
      { label: 'Airtable', slug: 'airtable', hex: '3AC1FF' },
      { label: 'Git', slug: 'git', hex: 'F05032' },
      { label: 'Notion', slug: 'notion', hex: 'EDEDED' },
      { label: 'Obsidian', slug: 'obsidian', hex: '7C3AED' },
      { label: 'GoHighLevel', svgKey: 'gohighlevel' },
      { label: 'Smartlead', icon: 'send' },
      { label: 'Instantly', icon: 'mail' },
    ],
  },
];
...
function LogoChip({ item }) {
  return (
    <span style={{
      ...
      color: 'var(--text-primary)', background: 'var(--bg-raised)', border: '1px solid var(--border)', whiteSpace: 'nowrap',
    }}>
      {item.svgKey
        ? <span style={{ display: 'block', flexShrink: 0, lineHeight: 0 }}>{CUSTOM_SVG[item.svgKey](14)}</span>
        : item.slug
          ? <img src={`https://cdn.simpleicons.org/${item.slug}/${item.hex}`} width={14} height={14} alt="" loading="lazy" style={{ display: 'block', flexShrink: 0 }} />
          : <Icon name={item.icon} size={14} color="var(--accent-fg)" />}
      {item.label}
    </span>
  );
}

function StackCard({ g, i }) {
  return (
    <Reveal delay={Math.min(i, 4) * 70} variant="scale">
      <Card hover={false} style={{ height: '100%', padding: 20 }}>
        <p style={{
          fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em',
          color: 'var(--accent-fg)', textTransform: 'uppercase', margin: '0 0 14px',
        }}>{g.title}</p>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
          {g.items.map((it) => <LogoChip key={it.label} item={it} />)}
        </div>
      </Card>
    </Reveal>
  );
    ...
    <Reveal delay={Math.min(i, 5) * 50}>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <span style={{
            fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 15, color: 'var(--text-primary)',
            display: 'block', lineHeight: 1.3,
          }}>{c.name}</span>
          <span style={{ fontFamily: 'var(--font-body)', fontSize: 13, color: 'var(--text-faint)' }}>{c.issuer}</span>
        </div>

        {/* brand logo */}
        <img
          src={`https://cdn.simpleicons.org/${c.slug}/${c.hex}`}
          width={20} height={20} alt={c.issuer}
          loading="lazy"
          style={{ display: 'block', flexShrink: 0, opacity: 0.8 }}
        />

        {/* verify arrow on real certs */}
        {!isPlaceholder && (
          <Icon name="arrowUpRight" size={14} color="var(--text-faint)" />
        )}
      </a>
    </Reveal>
  );
...
function Skills() {
  ...
  return (
    <section id="skills" className="fullbleed" style={{ paddingTop: 96, paddingBottom: 96 }}>
      <div className="fullbleed-inner">

      {/* ── heading ───────────────────────────────────────── */}
      <Reveal variant="fade" style={{ marginBottom: 32 }}>
        <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 'clamp(26px, 3.5vw, 40px)', color: 'var(--text-primary)', margin: '0 0 0' }}>
          Build, grow, and systemise.
        </h2>
      </Reveal>

      {/* ── two-column: strengths + tech stack ────────────── */}
      <div className="skills-layout" style={{ display: 'grid', gridTemplateColumns: '200px 1fr', gap: 32, alignItems: 'start' }}>
        {/* left — strengths */}
        <aside>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            <Icon name="sparkles" size={16} color="var(--accent)" />
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', margin: 0 }}>Strengths</h3>
          </div>
          <div style={{ display: 'flex', flexDirection: mobile ? 'row' : 'column', flexWrap: mobile ? 'wrap' : 'nowrap', alignItems: 'flex-start', gap: 8 }}>
            {STRENGTHS.map((s) => (
              <span key={s} style={{
                padding: '7px 14px', borderRadius: 999, fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500,
                color: 'var(--accent-fg)', background: 'var(--accent-10)', border: '1px solid var(--accent-30)',
              }}>{s}</span>
            ))}
          </div>
        </aside>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
            ...
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', margin: 0 }}>Tech stack</h3>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr' : 'repeat(2, 1fr)', gap: 14 }}>
            {STACK.map((g, i) => <StackCard key={g.title} g={g} i={i} />)}
          </div>
        </div>
      </div>
      ...
      <div style={{ marginTop: 64 }}>
        <Reveal>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <Icon name="award" size={16} color="var(--accent)" />
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 16, color: 'var(--text-primary)', margin: 0 }}>Certifications</h3>
          </div>
        </Reveal>

        <div style={{ marginTop: 8 }}>
          {CERTS.map((c, i) => <CertRow key={c.name} c={c} i={i} />)}
        </div>
      </div>

      <style dangerouslySetInnerHTML={{ __html: '@media (max-width: 720px){ .skills-layout{ grid-template-columns: 1fr !important; } }' }} />
      </div>
    </section>
  );
}
