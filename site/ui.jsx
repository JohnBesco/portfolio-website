...
function useMobile(breakpoint) {
  ...
  useEffect(function () {
    ...
    function check() {
      clearTimeout(timer);
      timer = setTimeout(function () { setMobile(window.innerWidth < breakpoint); }, 100);
    }
    window.addEventListener('resize', check);
    return function () { window.removeEventListener('resize', check); clearTimeout(timer); };
  }, []);
  return mobile;
}
...
/* ─── Button ─────────────────────────────────────────────── */
function Button({ variant = 'primary', size = 'md', children, onClick, type, href, target }) {
  const [hover, setHover] = useState(false);
  const sizes = {
    sm: { fontSize: 14, padding: '6px 12px' },
    md: { fontSize: 16, padding: '10px 18px' },
    lg: { fontSize: 18, padding: '13px 24px' },
  };
  const variants = {
    primary: { background: hover ? 'var(--accent-90)' : 'var(--accent)', color: '#0F0F0F', border: '1px solid transparent' },
    secondary: { background: hover ? 'var(--bg-card)' : 'transparent', color: 'var(--text-primary)', border: `1px solid ${hover ? 'var(--accent-30)' : 'var(--border)'}` },
    ghost: { background: hover ? 'var(--bg-card)' : 'transparent', color: hover ? 'var(--text-primary)' : 'var(--text-secondary)', border: '1px solid transparent' },
  };
  const sharedStyle = {
    display: 'inline-flex', alignItems: 'center', gap: 8, fontFamily: 'var(--font-body)',
    fontWeight: 500, borderRadius: 8, cursor: 'pointer', whiteSpace: 'nowrap', textDecoration: 'none',
    transition: 'background-color 160ms cubic-bezier(0.4,0,0.2,1), border-color 160ms cubic-bezier(0.4,0,0.2,1), color 160ms cubic-bezier(0.4,0,0.2,1)', ...sizes[size], ...variants[variant],
  };
  if (href) {
    return (
...
function Badge({ variant = 'default', children }) {
  const v = {
    default: { background: 'var(--badge-bg)', color: 'var(--badge-text)', border: '1px solid var(--badge-border)' },
    primary: { background: 'var(--accent-10)', color: 'var(--accent-fg)', border: '1px solid var(--accent-30)' },
    success: { background: 'color-mix(in srgb, var(--color-success) 10%, transparent)', color: 'var(--color-success)', border: '1px solid color-mix(in srgb, var(--color-success) 20%, transparent)' },
  };
  return (
    <span style={{
      display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 12px',
      borderRadius: 999, fontFamily: 'var(--font-body)', fontSize: 12, fontWeight: 500,
      whiteSpace: 'nowrap', ...v[variant],
    }}>{children}</span>
  );
}

/* ─── Card ───────────────────────────────────────────────── */
function Card({ children, hover = true, style, ...rest }) {
  const [h, setH] = useState(false);
  return (
    <div onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)} {...rest}
      style={{
        background: hover && h ? 'var(--bg-raised)' : 'var(--bg-card)',
        border: `1px solid ${hover && h ? 'var(--border-active)' : 'var(--border)'}`,
        borderRadius: 12, padding: 24, transition: 'background-color 160ms cubic-bezier(0.4,0,0.2,1), border-color 160ms cubic-bezier(0.4,0,0.2,1)',
        ...style,
      }}>{children}</div>
  );
}
...
function Reveal({ children, delay = 0, style, variant = 'rise' }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { root: scrollRoot || null, threshold: 0.05, rootMargin: '0px 0px -40px 0px' }
    );
    observer.observe(el);
  ...
  return (
    <div ref={ref} style={{
      ...
      transition: visible
        ? `opacity 480ms ease-out ${delay}ms, transform 480ms cubic-bezier(0,0,0.2,1) ${delay}ms`
        : 'none',
      ...style,
    }}>
      {children}
...
function Stat({ value, suffix = '', label }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 32, color: 'var(--accent-fg)', lineHeight: 1 }}>{value}{suffix}</span>
...
function Overline({ children, style }) {
  return (
    <p style={{
      fontFamily: 'var(--font-body)', fontSize: 12, fontWeight: 500, textTransform: 'uppercase',
      letterSpacing: '0.08em', color: 'var(--text-muted)', margin: 0, ...style,
    }}>{children}</p>
  );
}
...
function HoverButton({ children, onClick, primary = false }) {
  ...
  const createCircle = (x, y) => {
    ...
    setCircles((prev) => [...prev, { id: Date.now() + Math.random(), x, y, color, fade: null }]);
  };
  ...
  useEffect(() => {
    circles.forEach((c) => {
      if (!c.fade) {
        setTimeout(() => setCircles((p) => p.map((x) => x.id === c.id ? { ...x, fade: 'in' } : x)), 0);
        setTimeout(() => setCircles((p) => p.map((x) => x.id === c.id ? { ...x, fade: 'out' } : x)), 1000);
        setTimeout(() => setCircles((p) => p.filter((x) => x.id !== c.id)), 2200);
      }
    });
  }, [circles]);

  const innerGlow = primary
    ? '0 0 0 1px rgba(146,208,252,0.45), inset 0 0 18px 0 rgba(146,208,252,0.28), inset 0 -3px 12px 0 rgba(146,208,252,0.30)'
    : 'inset 0 0 0 1px rgba(170,202,255,0.20), inset 0 0 16px 0 rgba(170,202,255,0.10), inset 0 -3px 12px 0 rgba(170,202,255,0.15)';

  return (
    <button ref={btnRef} type="button" onClick={onClick} className="hb"
      onPointerMove={onMove}
      onPointerEnter={() => setListening(true)}
      onPointerLeave={() => setListening(false)}
      style={{
        position: 'relative', isolation: 'isolate', display: 'inline-flex', alignItems: 'center',
        gap: 9, padding: '13px 24px', borderRadius: 999, border: 'none', cursor: 'pointer',
        fontFamily: 'var(--font-body)', fontWeight: 500, fontSize: 15, lineHeight: 1.2,
      ...
      {circles.map(({ id, x, y, color, fade }) => (
        <span key={id} aria-hidden="true" style={{
          position: 'absolute', left: x, top: y, width: 12, height: 12,
          transform: 'translate(-50%,-50%)', borderRadius: 999, filter: 'blur(10px)',
      ...
      <span className="hb-border" aria-hidden="true" style={{
...
function WavePath({ style }) {
  ...
  const getWidth = () =>
    containerRef.current ? containerRef.current.offsetWidth : window.innerWidth * 0.9;
  ...
  const manageMouseEnter = () => {
    if (reqIdRef.current) { cancelAnimationFrame(reqIdRef.current); resetAnimation(); }
  };
  ...
  const resetAnimation = () => { timeRef.current = Math.PI / 2; progressRef.current = 0; };

  return (
    <div ref={containerRef} style={{ position: 'relative', height: 1, width: '100%', ...style }}>
      <div
        onMouseEnter={manageMouseEnter}
        onMouseMove={manageMouseMove}
        onMouseLeave={manageMouseLeave}
        style={{ position: 'relative', top: -20, zIndex: 10, height: 40, width: '100%', cursor: 'default' }}
      />
      <svg style={{ position: 'absolute', top: -100, height: 300, width: '100%', overflow: 'visible', pointerEvents: 'none' }}>
        <path ref={pathRef} style={{ fill: 'none', stroke: 'var(--border)', strokeWidth: 1.5 }} />
      </svg>
    </div>
  );
}

Object.assign(window, { Icon, Button, Badge, Card, Reveal, Stat, Overline, inView, HoverButton, WavePath });
