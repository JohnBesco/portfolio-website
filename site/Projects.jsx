...
const AI_BRAIN = {
  ...
  features: [
    '50+ reusable Claude skills across dev, marketing, and content workflows',
    'Project-level context files, operating rules, and memory layers',
    'Session handoff flows that keep work coherent across conversations',
    'Custom hooks, cron triggers, and structured delivery SOPs',
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
            <Overline style={{ color: 'var(--accent-fg)' }}>{p.meta}</Overline>
            <span style={{
              fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600,
              letterSpacing: '0.06em', color: 'var(--accent-fg)', background: 'var(--accent-10)',
              border: '1px solid var(--accent-30)', borderRadius: 999, padding: '3px 10px',
            }}>Private System</span>
          </div>

          {/* Title */}
          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: 26, color: 'var(--text-primary)', margin: '0 0 10px', letterSpacing: '-0.01em' }}>
            {p.name}
          </h3>

          {/* Description */}
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.65, margin: '0 0 18px', maxWidth: 640 }}>
            {p.desc}
          </p>
    ...
    <Reveal variant="scale" style={{ marginBottom: 20 }}>
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1, flex: 1, minWidth: 0 }}>
          <Overline style={{ marginBottom: 10 }}>{p.meta}</Overline>
          <h3 style={{
            fontFamily: 'var(--font-display)', fontWeight: 800,
            fontSize: 26, color: 'var(--text-primary)',
            margin: '8px 0 10px', letterSpacing: '-0.01em', lineHeight: 1.2,
          }}>{p.name}</h3>
          <p style={{
  ...
  return (
    <Reveal delay={Math.min(i, 4) * 70} variant="scale">
      ...
      >
        ...
        {/* Accent gradient glow — fades in on hover */}
        <div style={{
          position: 'absolute', inset: 0, pointerEvents: 'none', borderRadius: 12,
          opacity: hover ? 1 : 0, transition: 'opacity 300ms',
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%' }}>
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
            ...
            {isLive && (
              <a href={p.url} target="_blank" rel="noreferrer" aria-label={`Open ${p.name}`}
                style={{ display: 'inline-flex', color: hover ? 'var(--accent-fg)' : 'var(--text-muted)', transition: 'color 160ms', marginTop: 2, padding: 6, margin: -6 }}>
                <Icon name="arrowUpRight" size={18} color="currentColor" />
              </a>
            )}
          </div>

          <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 22, color: 'var(--text-primary)', margin: '10px 0 8px' }}>
            {p.name}
          </h3>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginTop: 'auto', paddingTop: 18 }}>
            {isLive ? (
              <a href={p.url} target="_blank" rel="noreferrer"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--accent-fg)' }}>
                <Icon name="externalLink" size={14} color="currentColor" />{p.live}
              </a>
            ) : (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text-muted)' }}>
            ...
            {p.repo && (
              <a href={p.repo} target="_blank" rel="noreferrer"
                style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text-secondary)' }}>
                <Icon name="github" size={14} color="currentColor" />Code
              </a>
            )}
          </div>
        </div>
...
function Projects() {
  ...
  return (
    <section id="projects" className="fullbleed" style={{ paddingTop: 96, paddingBottom: 96 }}>
      <div className="fullbleed-inner">

        {/* Section header */}
        <Reveal variant="fade" style={{ marginBottom: 40 }}>
          <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: mobile ? 28 : 'clamp(26px, 3.5vw, 40px)', color: 'var(--text-primary)', margin: '0 0 8px' }}>
            Pages, campaigns, and systems that ship.
          </h2>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 15, color: 'var(--text-secondary)', margin: 0, maxWidth: 560 }}>
            Client builds and side projects across web, growth, and AI.
          </p>
        </Reveal>

        {/* ── Client Work ─────────────────────────────────── */}
        <div style={{ marginBottom: 12 }}>
          <p style={{ fontFamily: 'var(--font-body)', fontSize: 13, fontWeight: 500, color: 'var(--text-tertiary)', margin: '0 0 16px' }}>
            Client work
          </p>
          <ClientFeaturedCard p={CLIENT_PROJECTS[0]} />
          <div style={{ display: 'grid', gridTemplateColumns: cols, gap: 20 }}>
            {CLIENT_PROJECTS.slice(1).map((p, i) => <ProjectCard key={p.name} p={p} i={i} />)}
          </div>
        </div>

        {/* ── Divider ─────────────────────────────────────── */}
        <WavePath style={{ margin: '52px 0' }} />
        ...
        <div>
          ...
          <div style={{ marginBottom: 20 }}>
            <FeaturedProjectCard p={AI_BRAIN} />
          </div>
          {SIDE_PROJECTS.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: cols, gap: 20 }}>
              {SIDE_PROJECTS.map((p, i) => <ProjectCard key={p.name} p={p} i={i} />)}
            </div>
          )}
        </div>
